...
import static org.neo4j.ogm.autoindex.IndexType.COMPOSITE_INDEX;
import static org.neo4j.ogm.autoindex.IndexType.NODE_KEY_CONSTRAINT;
import static org.neo4j.ogm.autoindex.IndexType.NODE_PROP_EXISTENCE_CONSTRAINT;
import static org.neo4j.ogm.autoindex.IndexType.SINGLE_INDEX;
import static org.neo4j.ogm.autoindex.IndexType.UNIQUE_CONSTRAINT;
...
class AutoIndex {
    ...
    private String createDescription(IndexType type, String owningType, String[] properties) {
        ...
        switch (type) {
            ...
            case NODE_PROP_EXISTENCE_CONSTRAINT:
                if (properties.length != 1) {
                    throw new IllegalArgumentException(NODE_PROP_EXISTENCE_CONSTRAINT + " must have exactly one property, got " +
                        Arrays.toString(properties));
                }
    ...
    public static Optional<AutoIndex> parse(String description) {
        ...
        pattern = Pattern.compile("INDEX ON :(?<label>.*)\\((?<property>.*)\\)");
    ...
    public boolean equals(Object o) {
        ...
        if (!Arrays.equals(properties, autoIndex.properties)) return false;
        if (!owningType.equals(autoIndex.owningType)) return false;
